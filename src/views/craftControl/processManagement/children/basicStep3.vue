 ...
 * @Last Modified time: 2019-05-15 10:48:14
 */
<!--选择设备类型-->
<template>
  <div class="basicStep3">
    <div class="el-card box-card is-always-shadow">
      <div class="el-card__header fz16">
        <el-button type="primary" size="small" @click="selectData" class="mr-3">选择设备</el-button>
      </div>
    ...
    <!-- 选择设备信息 -->
    <el-dialog title="选择设备信息" :visible.sync="visibleDialog" width="400px" :close-on-click-modal='false'>
      <el-row type="flex" justify="end" class="mb-3">
        <el-input placeholder="搜索设备类型关键字" style="width:300px" class="mr-3" clearable v-model="filters1.keyWord" @clear="selectInput" @change="selectInput" @keyup.native.enter="selectInput">
      ...
      <el-table :data="list" ref="multipleTable" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="device_type_name" label="设备类型名称" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    //获取弹出框table数据
    async getListData() {
      var res = await this.ajax("selectDeviceTypeAll", this.filters1);
      if (res.status == 0) {
        this.list = res.map.deviceTypes;
